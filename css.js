...
define('require/css', ['require/normalize'], function(normalize) {
  ...
  if (!engine) {}
    ...
    hackLinks = parseInt(engine[1], 10) < 6 || parseInt(engine[7], 10) <= 9;
    engine = 'trident';
  }
  else if (engine[2]) {
    ...
    hackLinks = parseInt(engine[4], 10) < 18;
    engine = 'gecko';
  }
  else if (testing) {
  ...
  cssAPI.setBuffer = function(css, isLess) {
    ...
    for (var i = 0; i < curBuffer.length; i++) {
      // find the resources in the less or css buffer depending which one this is
